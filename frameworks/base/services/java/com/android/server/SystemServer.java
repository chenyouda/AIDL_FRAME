...
import com.android.server.MouseService;
import android.content.Intent;
    ...
    public void initAndLoop() {
        ...
        if (factoryTest != SystemServer.FACTORY_TEST_LOW_LEVEL) {
           ...
           try {
            ...
            // MStar Android Patch Begin
            try {
                Slog.i(TAG, "Mouse Service");
                mouse = new MouseService(context);
                ServiceManager.addService(Context.MOUSE_SERVICE, mouse);
            } catch (Throwable e) {
                reportWtf("starting Mouse service", e);
            }
            // MStar Android Patch End
        ...
        contextF.registerReceiver(new MasterClearReceiver(), filter,"android.permission.MASTER_CLEAR",null);
        //CTV Patch End
        ...
        ActivityManagerService.self().systemReady(new Runnable() {
            public void run() {
                ...
                // MStar Android Patch Begin
                if ("sn".equals(SystemProperties.get("mstar.build.mstartv"))
                    || (("mi".equals(SystemProperties.get("mstar.build.mstartv")))
                    && ("true".equals(SystemProperties.get("mstar.build.for.stb"))))) {
                    Slog.i(TAG, "Startup TvService Apk....");
                    contextF.startService(new Intent("com.mstar.tv.service.TvService"));
                } else {
                    Slog.i(TAG, "ddi system do not need TvService Apk.");
                }
                // MStar Android Patch End
                try {
